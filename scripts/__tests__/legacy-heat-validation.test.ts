...
describe("legacy Heat authoring guard", () => {
  ...
  it("accepts only the constructs explicitly approved for legacy effect IDs", () => {
    expect(validateLegacyHeatContentRecord("content/cards/threats/crown-bell-baron.json", { id: "crown-bell-baron", effect: { type: "gain_heat", amount: 1 } })).toEqual([]);
    expect(validateLegacyHeatContentRecord("content/cards/threats/crown-bell-baron.json", { id: "crown-bell-baron", effect: { type: "lose_heat", amount: 1 } })[0]).toContain("lose_heat");
