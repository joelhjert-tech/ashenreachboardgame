...
describe("legacy Heat authoring guard", () => {
    it("retains only the seven effect and stable compatibility approval IDs", () => {
      expect(LEGACY_HEAT_EFFECT_APPROVALS).toHaveLength(2);
      expect(OTHER_LEGACY_HEAT_COMPATIBILITY_APPROVALS).toHaveLength(5);
  ...
  it("permits no imported board/scenario Heat effects after C3B and rejects additions", () => {
  ...
  it("accepts only the constructs explicitly approved for legacy effect IDs", () => {
    expect(validateLegacyHeatContentRecord("content/cards/threats/false-route-procession.json", { id: "false-route-procession", effect: { type: "gain_heat", amount: 1 } })[0]).toContain("gain_heat");
