...
| `scenario_mirror_of_false_heroes` | scenario / `src/game/data/scenarios.ts` | high-pressure confrontation: `gain_heat 1` | Face Yourself confrontation plan. | **APPROVED C3A: remove without replacement** | No | Content-only; implementation pending |
...
The effect remains authored until implementation, so the current population remains **7 occurrences across 7 IDs** and the audit verdict remains **FAIL**. After the approved implementation, the projected population is **6 occurrences across 6 IDs**: four escalations and two followers. The broader Mirror preparation/confrontation contract remains separate scenario-foundation work and is not a prerequisite for this narrow removal.
