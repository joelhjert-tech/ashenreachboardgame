...
| `escalation-blackstar-hunger` | escalation / `content/cards/escalations/escalation-blackstar-hunger.json` | resolve: `gain_heat_all 1` | The dark star pulls metal, courage, and breath into its gravity. | **IMPLEMENTED C4B1: removed without replacement** | No | Preserved existing `escalationDelta: 1`; no additional effect |
| `escalation-choir-feedback` | escalation / `content/cards/escalations/escalation-choir-feedback.json` | resolve: `gain_heat_all 1` | The relay choir turns every stable signal into a shriek. | **IMPLEMENTED C4B1: removed without replacement** | No | Preserved existing `escalationDelta: 1`; no additional effect |
| `escalation-marrow-surgery-debt` | escalation / `content/cards/escalations/escalation-marrow-surgery-debt.json` | resolve: `gain_heat 1` | Every healed Wound starts billing the future. | **IMPLEMENTED C4B2: removed without replacement** | No | Preserved existing `escalationDelta: 1`; no Salvage or other replacement |
| `escalation-saltwind-lockdown` | escalation / `content/cards/escalations/escalation-saltwind-lockdown.json` | resolve: `gain_heat_all 1` | Void-salt wind makes every easy crossing abrasive. | **IMPLEMENTED C4B1: removed without replacement** | No | Preserved existing `escalationDelta: 1`; no movement or other replacement |
...
C4B1 implementation status: **complete** for Group 1. `escalation-blackstar-hunger`, `escalation-choir-feedback`, and `escalation-saltwind-lockdown` have no `resolveEffect`; their existing `escalationDelta: 1` remains authoritative. Authored Heat is now **3 occurrences across 3 IDs**. Group 2 (`escalation-marrow-surgery-debt`) remains approved but unimplemented; both followers remain blocked. See `heat-compatibility-c4b1-shared-escalation-implementation.md`.

C4B2 implementation status: **complete** for Group 2 and for the full C4A escalation scope. `escalation-marrow-surgery-debt` has no `resolveEffect`; `escalationDelta: 1` remains authoritative, and the historical `lose_salvage 1` proposal was not introduced. Escalation-authored Heat is now **0**. Authored Heat is **2 occurrences across 2 blocked follower IDs**: `crownless-advocate` and `saltflat-bone-reader`. See `heat-compatibility-c4b2-marrow-escalation-implementation.md`.
