...
All 17 IDs are approved and implemented through H9B. H1 removal-only covers `cinder-gate-backlash`, `mirror-rot-interference`, and `webglass-snarefield`; H2 normal Wound pressure covers `choir-static-burst` and `lantern-moth-swarm`; H3 bounded floor-zero Salvage pressure covers `crown-bell-baron`, `pale-contract-collector`, and `soot-stained-cutpurse`; H4B exact-instance Equipment suppression covers `relay-husk` and `signal-rotted-engineer`; H4C covers `siren-relay-echo`; H5B covers `ashen-doppelganger` and `hymn-scarred-zealot`; H6B covers `false-route-procession`; H7B covers guarded Global Escalation for `gateblind-pulse`; H8B covers the same floor-zero Salvage model for `marrow-tax-auditors`; and H9B implements `memory-tax-gate` as a funded exact-Salvage-loss versus delayed non-battle-test penalty choice.
...
10. H9B: `memory-tax-gate` — IMPLEMENTED. When funded, the owner privately chooses exact 1 Salvage loss or next non-battle test `-1`; at zero Salvage the modifier applies automatically through one narrow persisted choice over the existing consequence lifecycles.
