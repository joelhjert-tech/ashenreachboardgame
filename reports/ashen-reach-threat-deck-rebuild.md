...
Final triage through H5A approves thirteen IDs. Eleven are implemented: H1 removal-only covers `cinder-gate-backlash`, `mirror-rot-interference`, and `webglass-snarefield`; H2 normal Wound pressure covers `choir-static-burst` and `lantern-moth-swarm`; H3 bounded floor-zero Salvage pressure covers `crown-bell-baron`, `pale-contract-collector`, and `soot-stained-cutpurse`; H4B exact-instance Equipment suppression covers `relay-husk` and `signal-rotted-engineer`; H4C covers `siren-relay-echo` with its paired next non-battle Command modifier. H5A approves `ashen-doppelganger` for 2 preventable Wounds and `hymn-scarred-zealot` for 1 preventable Wound, both awaiting H5B implementation and neither granting a direct Scar. Four IDs remain blocked: `false-route-procession`, `gateblind-pulse`, `marrow-tax-auditors`, and `memory-tax-gate`.
...
6. H5B severe Wound group: `ashen-doppelganger`, `hymn-scarred-zealot` — APPROVED in H5A for later implementation through the existing normal preventable-Wound lifecycle; no direct Scar and no infrastructure extension.
7. Keep `false-route-procession`, `gateblind-pulse`, `marrow-tax-auditors`, and `memory-tax-gate` blocked until their prerequisites in `reports/heat-retirement-implementation-plan.md` are approved.
